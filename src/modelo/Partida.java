...
public class Partida {
	...
	public Partida(int nf, int nc, int nb) {
		...
		iniciaDisparos(); //Inicia la matriz de disparos
	...
	public int pruebaCasilla(int f, int c) {
		...
		if(numero>=0) { // 	Has tocado un barco
			if(barcos.get(numero).tocaBarco()) {
				quedan--;
				return -3; //Si el barco está hundido
			}
			
			return -2;
		}
		return numero;	//agua -1
	}
	/**	public int pruebaCasilla(int f, int c) {
 int numero = mar[f][c];
 if(numero>=0) {
 ...
 return numero;
	...
	public boolean estaHundido(int idBarco) {
		return barcos.get(idBarco).isHundido();
	}
	/**
	...
	public String[] getSolucion() {
		int tamano = barcos.size();
		String[] devolver = new String[tamano];
		for(int i=0; i<tamano; i++) {
			devolver[i] = getBarco(i);
		}
		return devolver;
	}
